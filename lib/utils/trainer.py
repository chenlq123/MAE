...
def get_trainer(args, model, train_loader, optimizer, lr_scheduler, device, tfboard):
    ...
    if args.train.batch_size == 1:
        accumulation_steps = 5
    elif args.train.batch_size == 2:
        accumulation_steps = 3
    elif args.train.batch_size == 3:
        accumulation_steps = 2
    else:
        accumulation_steps = 1
    print('accumulation_steps: ', accumulation_steps)
    print('batch_size: ', args.train.batch_size)
    ...
    def _update_model(engine, data):
        ...
        if args.train.clip_gradient > 0:
            clip_grad_norm_(model.parameters(), args.train.clip_gradient)
        if engine.state.iteration % accumulation_steps == 0 or engine.state.iteration >= len(train_loader):
            optimizer.step()
            optimizer.zero_grad()
    ...
    def _post_epoch(engine):
        ...
        if is_main_process():
            if engine.state.epoch >= 10:
                save_name = osp.join(args.path, 'checkpoint' + str(engine.state.epoch) + '.pth')
            else:
